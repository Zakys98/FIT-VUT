                     ...
                     const S_Material &material)
{
  ...
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  S_Coords v(0.0 - aa.x, 0.0 - aa.y, -CAMERA_DIST - aa.z);

  double cos = v.x * nn.x + v.y * nn.y + v.z * nn.z;
...
void DrawScene()
{
  ...
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  double tx = obj_move_x * 0.015;
  double ty = obj_move_y * 0.015;
  double rx = obj_rot_x * 0.01;
  double ry = obj_rot_y * 0.01;

  trSetMatrix(get);

  trScale(-1.0, 1.0, 1.0);

  trTranslate(tx, ty, 0.0);
  trRotateX(rx);
  trRotateY(ry);
