...
void htInsert ( tHTable* ptrht, tKey key, tData data ) {
    ...
    if(tmp != NULL){
        ...
        tHTItem *insert = malloc(sizeof(tHTItem));
        if(insert == NULL) {
            return;
        }
        insert->data = data;
        insert->key = key;
        insert->ptrnext = (*ptrht)[hashCode(key)];
        (*ptrht)[hashCode(key)] = insert;
    }
    //solved = 0; /*v pripade reseni, smazte tento radek!*/
}
...
void htDelete ( tHTable* ptrht, tKey key ) {
    ...
    tHTItem *tmp = NULL;
    while(delete != NULL && strcmp(delete->key, key)){
        tmp = delete;
