...
void untilLeftPar(tStack *s, char *postExpr, unsigned *postLen) {
    char p;
    while (stackEmpty(s) == 0 && ((p = s->arr[s->top]) != '(')) {
        postExpr[*postLen] = p;
        (*postLen)++;
        stackPop(s);
    }

    // odstrani zbylou levou zavorku ze zasobniku
    if (s->arr[s->top] == '(') {
        stackPop(s);
    }
}
...
void doOperation(tStack *s, char c, char *postExpr, unsigned *postLen) {
    char t;
    // pokud je zasobnik prazdny, tak vlozi operator na vrchol zasobniku
    if (stackEmpty(s)) {
        stackPush(s, c);
        return;
    } else {
        // pokud tam je leva zavorka, tak vlozi operator na vrchol zasobniku
        stackTop(s, &t);
        if (t == '(') {
            stackPush(s, c);
            return;
    ...
    // pokud tam je operator s nizsi prioritou, tak vlozi operator na vrchol zasobniku
    if ((t == '+' || t == '-') && (c == '*' || c == '/')) {
        stackPush(s, c);
        return;
    } else {
        // pokud tam je operator s vyssi prioritou, tak odstrani operator z vrcholu zasobniku
        // a vlozi ho na konec retezce
        postExpr[*postLen] = t;
        (*postLen)++;
        stackPop(s);
...
char *infix2postfix(const char *infExpr) {
    ...
    stackInit(&s);
    char c;
    int i = 0; // index cteneho retezce
    // alokacce pameti pro vystupni retezec
    char *post = malloc(sizeof(char) * MAX_LEN);
    unsigned int postI = 0; // index vystupniho retezce
    if (post == NULL)
        return NULL;

    // naciteme znak ze vstupu a pracujeme s nim podle toho co to je
    while ((c = infExpr[i]) != '\0') {
        if (c == '+' || c == '-' || c == '*' || c == '/') {
            doOperation(&s, c, post, &postI);
        } else if (c == '('){
            stackPush(&s, '(');
        } else if (c == ')'){
            untilLeftPar(&s, post, &postI);
        } else if ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')){
            post[postI] = c;
            postI++;
        } else if (c == '='){
            while(!stackEmpty(&s)){
                stackTop(&s, &post[postI]);
                stackPop(&s);
                postI++;
            }
            post[postI] = '=';
